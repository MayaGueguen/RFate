...
test_that("PRE_FATE.params_PFGdrought gives error with wrong data : mat.PFG.tol", {
  ...
  expect_error(PRE_FATE.params_PFGdrought(name.simulation = "FATE_simulation"
                                          , mat.PFG.tol = data.frame(nameDist = "immediate", PFG = "A", responseStage = NA
                                                                     , killedIndiv = 1, resproutIndiv = 1))
               , "`mat.PFG.tol$responseStage` must contain numeric values", fixed = TRUE)
  expect_error(PRE_FATE.params_PFGdrought(name.simulation = "FATE_simulation"
                                          , mat.PFG.tol = data.frame(nameDist = "immediate", PFG = "A", responseStage = "a"
                                                                     , killedIndiv = 1, resproutIndiv = 1))
               , "`mat.PFG.tol$responseStage` must contain numeric values", fixed = TRUE)
  expect_error(PRE_FATE.params_PFGdrought(name.simulation = "FATE_simulation"
                                          , mat.PFG.tol = data.frame(nameDist = "immediate", PFG = "A", responseStage = factor(1)
                                                                     , killedIndiv = 1, resproutIndiv = 1))
               , "`mat.PFG.tol$responseStage` must contain numeric values", fixed = TRUE)
