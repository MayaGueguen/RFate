...
test_that("POST_FATE.graphic_mapPFGvsHS gives error with wrong data : name.simulation", {
  ...
  expect_error(POST_FATE.graphic_mapPFGvsHS("a")
               , "Wrong name folder given!\n `name.simulation` does not exist or does not contain a PARAM_SIMUL/ folder")
  expect_error(POST_FATE.graphic_mapPFGvsHS(factor("A"))
               , "Wrong name folder given!\n `name.simulation` does not exist or does not contain a PARAM_SIMUL/ folder")
  expect_error(POST_FATE.graphic_mapPFGvsHS(data.frame(1))
               , "Wrong name folder given!\n `name.simulation` does not exist or does not contain a PARAM_SIMUL/ folder")
...
test_that("POST_FATE.graphic_mapPFGvsHS gives error with wrong data : rasters", {
  ...
  expect_error(POST_FATE.graphic_mapPFGvsHS(name.simulation = "FATE_simulation"
                                            ...
                                            , year = 10
                                            , opt.strata = 1)
               , "Missing data!\n The folder FATE_simulation/RESULTS/Hello/BIN_perPFG_perStrata/ does not contain adequate files"
               , fixed = TRUE)
  
  file.create("FATE_simulation/RESULTS/Hello/BIN_perPFG_perStrata/Binary_YEAR_10_PFG1_STRATA_1.txt")
