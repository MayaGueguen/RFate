...
test_that(".getParam gives error with wrong data", {
  ...
  expect_error(.getParam(params.lines = "TEST_getParam.txt"), "`flag` must contain a character value of length > 0")
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = NA), "`flag` must contain a character value of length > 0")
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = NULL), "`flag` must contain a character value of length > 0")
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = 1), "`flag` must contain a character value of length > 0")
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = factor(1)), "`flag` must contain a character value of length > 0")
  ...
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = "PARAM1", flag.split = " ")
               , "`flag` (PARAM1) is not found within `params.lines` (TEST_getParam.txt)", fixed = T)
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = "PARAM1", flag.split = "^--.*--$")
               , "`flag` (--END_OF_FILE--) is not found within `params.lines` (TEST_getParam.txt)", fixed = T)
  write.table(data.frame("--END_OF_FILE--", 12), file = "TEST_getParam.txt", col.names = F, row.names = F, quote = F)
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = "PARAM1", flag.split = "^--.*--$")
               , "`flag.split` (^--.*--$) is not found within `params.lines` (TEST_getParam.txt)", fixed = T)
  write.table(data.frame(c("--T--", "--END_OF_FILE--"), 12), file = "TEST_getParam.txt", col.names = F, row.names = F, quote = F)
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = "PARAM1", flag.split = "^--.*--$")
               , "`flag.split` (^--.*--$) is not found within `params.lines` (TEST_getParam.txt)", fixed = T)
  write.table(data.frame(c("--T--", "--END_OF_FILE--")), file = "TEST_getParam.txt", col.names = F, row.names = F, quote = F)
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = "PARAM1", flag.split = "^--.*--$")
               , "`flag` (PARAM1) is not found within `params.lines` (TEST_getParam.txt)", fixed = T)
  
  write.table(data.frame(c("PARAM1", "--END_OF_FILE--")), file = "TEST_getParam.txt", col.names = F, row.names = F, quote = F)
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = "PARAM1", flag.split = "^--.*--$")
               , "`flag.split` (^--.*--$) is not found within `params.lines` (TEST_getParam.txt)", fixed = T)
  ...
  write.table(data.frame(c("PARAM1", "--END_OF_FILE--"), 12), file = "TEST_getParam.txt", col.names = F, row.names = F, quote = F)
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = "PARAM1", flag.split = "^--.*--$")
               , "`flag.split` (^--.*--$) is not found within `params.lines` (TEST_getParam.txt)", fixed = T)
  expect_error(.getParam(params.lines = "TEST_getParam.txt", flag = "PARAM1", flag.split = "^--.*--$", is.num = FALSE)
               , "`flag.split` (^--.*--$) is not found within `params.lines` (TEST_getParam.txt)", fixed = T)
...
test_that(".getParam gives correct output", {
  ...
  write.table(data.frame(c("--PARAM1--", 12, "--END_OF_FILE--")), file = "TEST_getParam.txt", col.names = F, row.names = F, quote = F)
  expect_equal(length(.getParam(params.lines = "TEST_getParam.txt", flag = "PARAM1", flag.split = "^--.*--$")), 1)
  ...
  write.table(data.frame(c("--PARAM1--", 12, 3, "--END_OF_FILE--")), file = "TEST_getParam.txt", col.names = F, row.names = F, quote = F)
  expect_equal(length(.getParam(params.lines = "TEST_getParam.txt", flag = "PARAM1", flag.split = "^--.*--$", is.num = FALSE)), 2)
  expect_equal(.getParam(params.lines = "TEST_getParam.txt", flag = "PARAM1", flag.split = "^--.*--$", is.num = TRUE), c(12,3))
