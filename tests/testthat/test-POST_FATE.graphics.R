...
## INPUTS
test_that("POST_FATE.graphics gives error with wrong data : no.years", {
  expect_error(POST_FATE.graphics(name.simulation = "FATE_simulation"
                                  , file.simulParam = "ParamSimul.txt"
                                  , year = 10)
               , "Wrong type of data!\n `no.years` must be an integer > 0"
               , fixed = TRUE)
...
## INPUTS
test_that("POST_FATE.graphics gives error with wrong data : folders / files : POST_FATE.graphic_evolutionPixels", {
  expect_error(POST_FATE.graphics(name.simulation = "FATE_simulation"
                                  , file.simulParam = "ParamSimul.txt"
                                  , year = 10
                                  , no.years = 1
...
test_that("POST_FATE.graphics gives error with wrong data : folders / files : POST_FATE.graphic_validationStatistics", {
  ...
  expect_error(POST_FATE.graphics(name.simulation = "FATE_simulation"
                                  ...
                                  , no.years = 1
                                  , opt.doFunc.evolutionCoverage = FALSE
                                  , opt.doFunc.evolutionPixels = FALSE)
...
test_that("POST_FATE.graphics gives error with wrong data : folders / files : POST_FATE.graphic_mapPFGcover", {
  expect_error(POST_FATE.graphics(name.simulation = "FATE_simulation"
               ...
               , "Missing data!\n The folder FATE_simulation/RESULTS/Hello/ABUND_perPFG_perStrata/ does not contain adequate files (number of strata null or no strata files found)"
               , fixed = TRUE)
})
...
test_that("POST_FATE.graphics gives right output", {
  expect_output(str(POST_FATE.graphics(name.simulation = "FATE_simulation"
                                       ...
                                       , opt.doFunc.mapPFGcover = FALSE
                                       , opt.doFunc.mapPFGlight = FALSE
                                       , opt.doFunc.mapPFGsoil = FALSE)), "List")
  expect_output(str(POST_FATE.graphics(name.simulation = "FATE_simulation"
                                       , year = 10
                                       , no.years = 1
                                       , opt.doFunc.evolutionCoverage = FALSE
