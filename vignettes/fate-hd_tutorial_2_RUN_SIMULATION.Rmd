---
title: "FATE-HD tutorial 2 - Run a simulation"
output:
  html_document:
    theme: lumen
    highlight: zenburn
...
While this is not mandatory to run a `FATE-HD` simulation, a specific and methodical folder architecture can be obtained with [the PRE_FATE.skeletonDirectory function](reference/PRE_FATE.skeletonDirectory.html) from the [RFate](https://mayagueguen.github.io/RFate/) package.
...
When all data and parameter files have been produced and correctly referred in a [SimulParameters](reference/PRE_FATE.params_simulParameters.html) file, a `FATE-HD` simulation can be run by just typing something like :
